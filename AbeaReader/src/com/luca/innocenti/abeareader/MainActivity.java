	...
	switch(id){
	 case R.id.it:
		...
		case R.id.uk:
	    	...
	    	if (minore != 0)
				{
				url = "http://www.nearbee.it/beacons/jsandroid3.php?uuid="+uuid+"&major="+Integer.toString(maggiore)+"&minor="+Integer.toString(minore)+"&lingua="+Integer.toString(lingua);
				visualizza(url);
				}
		  ...
		  //private String beaconid;
			...
			public void didRangeBeaconsInRegion(Collection<IBeacon> iBeacons,Region region) {
               ...
               double vicino = 10.0;
				...
				if (iBeacons.size() > 0) {
					// cerca tra tutti i beacon visti
					for (IBeacon iBeacon : iBeacons) {
						//e guarda quale e' il piu' vicino
					...
					// se il piu' vicino e' minore di 5 controlla se non e' gia' stato osservato
					// in caso affermativo lancia la pagina web
					if (vicino <= 5)
						{
						...
						//altrimenti se tutti beacon sono lontani lancia la pagina home
						// che risiede su assets
						else
						{
						visualizza("file:///android_asset/www/index.html");
						}
